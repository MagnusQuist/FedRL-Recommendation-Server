...
from app.db.models import CentralizedModel, FederatedModel
...
def load_pretrained_weights(path: Path = PRETRAINED_WEIGHTS_PATH) -> dict[str, np.ndarray]:
    if path.exists():
        data = np.load(path)
...
async def _federated_backbone_exists() -> bool:
    async with AsyncSessionLocal() as db:
        result = await db.execute(
            select(FederatedModel).where(FederatedModel.version == INITIAL_VERSION).limit(1)
        )
...
async def _centralized_backbone_exists() -> bool:
    async with AsyncSessionLocal() as db:
        result = await db.execute(
            select(CentralizedModel).where(CentralizedModel.version == INITIAL_VERSION).limit(1)
        )
        return result.scalar_one_or_none() is not None


async def seed_federated_backbone() -> None:
    """Insert v1 federated backbone row if missing."""
    if await _federated_backbone_exists():
        logger.info("Federated backbone version=%d already exists — skipping.", INITIAL_VERSION)
        return
    ...
    async with AsyncSessionLocal() as db:
        row = FederatedModel(version=INITIAL_VERSION, weights_blob=blob)
        db.add(row)
        await db.commit()
        await db.refresh(row)

    logger.info("Seeded federated backbone id=%d version=%d (blob=%d bytes).", row.id, row.version, len(blob))
...
async def seed_centralized_backbone() -> None:
    ...
    if await _centralized_backbone_exists():
        logger.info("Centralized backbone version=%d already exists — skipping.", INITIAL_VERSION)
        return
    ...
    logger.info("Seeded centralized backbone id=%d version=%d (blob=%d bytes).", row.id, row.version, len(row.backbone_blob))
