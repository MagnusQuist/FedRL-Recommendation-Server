...
def _load_categories(data_dir: Path) -> list[dict[str, Any]]:
    ...
    return [
        {"category_id": _to_int(row["category_id"]), "name": _to_str(row["name"]), "slug": _to_str(row["slug"])}
        for row in rows
    ]
...
def _load_food_item_categories(data_dir: Path) -> list[dict[str, Any]]:
    ...
    return [
        {"product_id": _to_str(row["product_id"]), "category_id": _to_int(row["category_id"])}
        for row in rows
    ]
...
def _load_substitution_groups(data_dir: Path) -> list[dict[str, Any]]:
    ...
    return [
        {"substitution_group_id": _to_int(row["substitution_group_id"]), "name": _to_str(row["name"])}
        for row in rows
    ]
...
def _load_substitution_group_items(data_dir: Path) -> list[dict[str, Any]]:
    ...
    return [
        {"substitution_group_id": _to_int(row["substitution_group_id"]), "product_id": _to_str(row["product_id"])}
        for row in rows
    ]
...
async def _reflect_tables(engine: AsyncEngine) -> MetaData:
    ...
    async with engine.begin() as conn:
        await conn.run_sync(lambda sync_conn: metadata.reflect(bind=sync_conn, only=TABLE_NAMES))
    return metadata
...
async def seed_catalogue() -> None:
    ...
    async with engine.begin() as conn:
        ...
        if substitution_group_items:
            await conn.execute(substitution_group_items_table.insert(), substitution_group_items)
